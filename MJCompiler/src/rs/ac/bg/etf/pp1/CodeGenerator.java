...
public class CodeGenerator extends VisitorAdaptor {
	...
	public void visit(DesignAssignopExpr expr) {
			...
			if(!this.pristupElem.isEmpty())this.pristupElem.pop();
			if(obj.getType().getKind()==Struct.Array) {
				...
				if(vector_dst!=null) 
				{
					...
					secondVector=null;
					return;
				} // vec su smestene vrednosti u vector
	...
	Stack<Boolean>  pristupElem=new Stack<Boolean>();
	
	public void visit(ElemLBracket el) {
		pristupElem.set(pristupElem.size()-1,true);
	...
	boolean isScalarOnStack=false;
	
	public void visit(DesigFactor factor) {
		
		boolean pristupElem= this.pristupElem.pop();
	...
	public void visit(ElemArray el) {
		if(el.getParent().getClass()==DesignAssignopExpr.class) {
			vector_dst=null;
		}
	}
	...
	public void visit(TermContMulop mulop) {
		...
		case "mul":
		{
			if(firstVector!=null && secondVector!=null){
				//provera da li su jednakih duzina
				Code.load(firstVector);
				Code.put(Code.arraylength);
				Code.load(secondVector);
				Code.put(Code.arraylength);
				Code.putFalseJump(Code.ne,0);
				int JumpifEqual=Code.pc-2;
			//ako nisu izuzetak
				Code.put(Code.trap);  
				Code.fixup(JumpifEqual);
			//ako jesu obrada
			  ...
			  Code.put(Code.pop);
			  if(isScalarOnStack) { Code.put(Code.mul);isScalarOnStack=false; }	  
			  firstVector=null;
			  secondVector=null;
			}else 
			if(firstVector!=null && vector_dst!=null){
				//mnozenje tipa scalar * vector
				
				//alociranje prostora za vector_dst!
					Code.load(firstVector);
					Code.put(Code.arraylength);
					Code.put(Code.newarray);
					Code.loadConst(1);
					Code.store(vector_dst);
		...
		//endloop
					Code.fixup(fixPcJmp);
					Code.put(Code.pop); //da se izbrisu zadnje dve vrednosti na steku
					Code.put(Code.pop);
					
					firstVector=vector_dst;
			...
			else if(firstVector!=null && vector_dst==null) {
				isScalarOnStack=true;
			}else {
				Code.put(Code.mul);		
				}
